...
Tài liệu này mô tả chi tiết, ở mức code, hai luồng tương tác chính giữa
người dùng — trình soạn thảo (ProseMirror/Yjs) — AI agent trong
...
Tài liệu chỉ tập trung vào nhánh backend đang chạy thật:
**`AGENT_BRAIN_MODE=python`** — Python (`agent_server.py`) quyết định gọi
tool, Node thực thi tool thật qua `DocumentToolRuntime`. Nhánh dự phòng
`AGENT_BRAIN_MODE=tanstack` (dùng `@tanstack/ai` thuần TypeScript) không
được đề cập ở đây.
...
`continue` / `insert` / `rewrite` **không phải** một lựa chọn trên UI. Điều
duy nhất frontend gửi lên là hình dạng của vùng chọn hiện tại:
...
`cursor` hay `selection` (xem mục "Sơ đồ" bên dưới).
...
"tiến trình chat" là hai cơ chế tách biệt hoàn toàn.
...
Hai đường đi (3-4-5-6) và (7-8) chạy **song song**: (3-4-5-6) là đường thực
thi + ghi dữ liệu thật; (7-8) là đường hiển thị tiến trình chat, không ghi gì
vào tài liệu.
...
| File | Vai trò |
|---|---|
| `src/routes/api/chat.ts` | `agentResponseStream` — tạo `DocumentToolRuntime`, build system prompt, gọi `runPythonAgentStream`, dọn dẹp |
...
| `src/lib/agent/prompts.ts` | `buildChatToolSystemPrompt(mode)`, `buildEditorContextSystemPrompt({editorContext, selectedText})`, `MODE_INSTRUCTIONS` |
...
| `src/lib/agent/documentToolDispatch.ts` | `executeDocumentTool`, `DOCUMENT_TOOL_SCHEMAS` — bảng schema + custom event của cả 17 tool |
| `src/lib/agent/pythonBridgeRegistry.ts`, `src/routes/api/agent-bridge/tool.ts` | Đăng ký `DocumentToolRuntime` theo `runId`, xác thực `AGENT_BRIDGE_SECRET`, thực thi tool thật |
...
| File | Vai trò |
|---|---|
| `src/routes/api/agent-python-demo/agent_server.py` | `stream_agent_run` — vòng lặp tool-calling streaming thật (OpenAI), `MUTATING_TOOLS`, fallback tự tóm tắt cho chat |
| `src/routes/api/agent-python-demo/agent_shared.py` | Schema `pydantic` cho 17 tool, `build_openai_tools()` |

---

## Luồng 1 — Toàn văn bản (không có vùng chọn)

**Kịch bản ví dụ:** người dùng đặt con trỏ ở cuối tài liệu, gõ "viết tiếp
thêm một đoạn" trong chat, không bôi đen gì.
...
### Các bước chính (đối chiếu file:hàm)

1. **Bắt ngữ cảnh** — `SelectionContextWatcher` (`CollaborativeEditor.tsx:160-189`)
   thấy `editorState.selection.empty === true` → phát `{kind:'cursor', anchor}`.
2. **Đóng băng + hiển thị mục tiêu** — khi composer focus, `freezeEditorContext`
   giữ nguyên context đã gửi; `chatTargetOverlay.ts` vẽ caret tại vị trí đó.
3. **Gửi request** — `createDurableChatConnection.ts` tự chèn `editorContext`
   vào `data`; POST tới `/api/chat`.
4. **Áp dụng context** — `DocumentToolRuntime.applyEditorContext`
   (`documentToolRuntime.ts:409-414`, nhánh `cursor`): xoá vùng chọn cũ nếu
   có, đặt `cursorAnchorBytes`.
5. **System prompt** — `buildEditorContextSystemPrompt` nhánh cursor
   (`prompts.ts:73-77`): "dùng cursor hiện tại, gọi `get_cursor_context` hoặc
   `get_document_snapshot` nếu cần thêm ngữ cảnh".
6. **Đọc toàn văn bản** — model gọi `get_document_snapshot`
   (`documentToolRuntime.ts:439-453`) — đây chính là bước "tóm tắt/đọc qua
   toàn bộ văn bản" trong luồng này.
7. **Bắt đầu streaming edit** — `start_streaming_edit(mode:'continue'|'insert')`
   (`documentToolRuntime.ts:941-952`) — không xoá gì, chỉ neo vị trí chèn.
8. **Chặn & ghi thật** — `routeAgentStreamChunks` (`chatStreamRouting.ts:118-151`)
   phát hiện `runtime.isStreamingEditActive()===true`, chuyển từng
   `TEXT_MESSAGE_CONTENT` thành lời gọi `runtime.pushStreamingText(delta)`
   (ghi vào Yjs thật) + một `CUSTOM streaming-insert-delta` (chỉ để debug UI).
9. **Tóm tắt & dọn dẹp** — nếu chưa có câu chat khép lại
   (`chat_message_sent=False` trong `agent_server.py`), Python tự gọi thêm
   một lượt OpenAI không kèm tool để tóm tắt; `chat.ts` dọn `stopStreamingEdit`
   / `releaseBridgeRun` / `runtime.destroy()` trong `finally`.
...
duy nhất, không có giai đoạn streaming:
...
- `set_format` (`documentToolRuntime.ts:687-796`) — **bắt buộc phải có
  ...
  active selection'`.
- `replace_matches` — thay thế nhiều đoạn khớp cùng lúc.
- `delete_selection` — xoá vùng đang chọn (no-op nếu không có selection).
...
1. **Bắt vùng chọn** — `SelectionContextWatcher` thấy `selection.empty === false`
   → phát `{kind:'selection', anchor, head}` (cả hai đều là Yjs relative
   position, sống sót qua chỉnh sửa đồng thời của người khác).
2. **Hiển thị mục tiêu** — `chatTargetOverlay.ts` vẽ decoration inline
   highlight đúng đoạn đã chọn.
3. **Áp dụng context** — `applyEditorContext` nhánh `selection`
   (`documentToolRuntime.ts:397-408`): lưu `selectionStartBytes`/`selectionEndBytes`,
   đặt cursor = vị trí lớn hơn giữa `anchor`/`head`.
4. **System prompt trích dẫn văn bản** — `buildEditorContextSystemPrompt`
   nhánh `selection` (`prompts.ts:62-72`) **nhúng thẳng nội dung đã chọn**
   (tối đa 240 ký tự) vào prompt, khác hẳn luồng 1 (luồng 1 không bao giờ
   nhúng nội dung tài liệu vào prompt, chỉ hướng dẫn gọi tool).
5. **Model chọn 1 trong 2 hướng**: sửa tức thời bằng tool đơn (không streaming)
   hoặc `start_streaming_edit(mode:'rewrite')` cho các bài viết lại dài hơn.
6. **`rewrite` bắt buộc có selection** — `documentToolRuntime.ts:925-940`
   ném lỗi `'Rewrite requires an active selection'` nếu gọi mà không có
   vùng chọn; nếu hợp lệ, xoá ngay lập tức vùng chọn và neo điểm chèn tại
   đúng vị trí vừa xoá (khác luồng 1: không xoá gì trước khi chèn).
7. **An toàn khi huỷ giữa chừng** — `stopStreamingEdit`
   (`documentToolRuntime.ts:1178-1186`): nếu bị huỷ trước khi có bất kỳ ký
   tự nào được chèn, tự động chèn lại đúng văn bản đã xoá (`deletedSelectionText`)
   — cơ chế này chỉ tồn tại cho `rewrite`, không áp dụng cho `continue`/`insert`.
...
Nguồn: `src/lib/agent/documentToolDispatch.ts`.
